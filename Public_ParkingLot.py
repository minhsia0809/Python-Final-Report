...
output_file("Public_ParkingLot_Counts.html")
...
p = figure(x_range=Public_P, y_range=(0, 10), plot_height=350, title="高雄市各地區公有停車場數量",
           toolbar_location=None, tools="")

p.vbar(x=dodge('Public_P', -0.25, range=p.x_range), top='立體', width=0.2, source=source,
       color="#c9d9d3", legend_label="立體")

p.vbar(x=dodge('Public_P',  0.0,  range=p.x_range), top='平面', width=0.2, source=source,
       color="#718dbf", legend_label="平面")

p.vbar(x=dodge('Public_P',  0.25, range=p.x_range), top='總數', width=0.2, source=source,
